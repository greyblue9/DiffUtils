...
def parse_unified_diff(text, lenient=False):
    ...
    def process_chunk(chunk, offset, expected_original, expected_revised):
        ...
        ), f"Invalid expected_original type: {type(expected_original)}"

        assert (
            type(expected_revised) is int
        ), f"Invalid expected_revised type: {type(expected_revised)}"
        ...
        if expected_original != actual_original:
            ...
            assert str(expected_original) != str(
                actual_original
            ), f"{repr(expected_original)}, {repr(actual_original)}"
        ...
        if expected_revised != actual_revised:
            ...
            assert str(expected_revised) != str(
                actual_revised
            ), f"{repr(expected_revised)}, {repr(actual_revised)}"
